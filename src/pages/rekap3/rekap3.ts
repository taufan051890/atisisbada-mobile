...
import { Rekap4Page } from '../rekap4/rekap4';
...
@IonicPage()
@Component({
  selector: 'page-rekap3',
  templateUrl: 'rekap3.html',
})
export class Rekap3Page {
  data : string;
  f : string;
  g : string;
  skpd : string;
  ...
  ionViewDidLoad(){
    ...
    loader.present().then(() => {
      this.http.get(this.GlobalProvider.url+'atis/pages/api/api/rekap/rekap.php?'+"&f="+this.f+"&g="+this.g)
      .map(result => result.json())
      .subscribe(data => {
        this.data = data.result;
        console.log(data.result);
        console.log('kontol'+ this.e1nama);
        loader.dismiss();
      },err => {
        console.log(err);
      }
