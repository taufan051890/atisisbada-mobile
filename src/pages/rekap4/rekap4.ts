...
import { Rekap5Page } from '../rekap5/rekap5';
...
@IonicPage()
@Component({
  selector: 'page-rekap4',
  templateUrl: 'rekap4.html',
})
export class Rekap4Page {
  data : string;
  f : string;
  g : string;
  h : string;
  ...
  ionViewDidLoad(){
    ...
    loader.present().then(() => {
      this.http.get(this.GlobalProvider.url+'atis/pages/api/api/rekap/rekap.php?'+"&f="+this.f+"&g="+this.g+"&h="+this.h)
      .map(result => result.json())
      .subscribe(data => {
